...
char *test_djb(){
    uint32_t hash = Hahsmap_djb_hash(&test1);
    mu_assert(hash != 0, "Bad hash.");

    hash = Hahsmap_djb_hash(&test2);
    mu_assert(hash != 0, "Bad hash.");

    hash = Hahsmap_djb_hash(&test3);
    mu_assert(hash != 0, "Bad hash.");
...
int gen_keys(DArray *keys, int num_keys) {
    int i = 0;
    FILE *urand = fopen("/Users/baz/VSProjects/zedc/c-skeleton/urandom", "r");
    check(urand != NULL, "Failed to open /Users/baz/VSProjects/zedc/c-skeleton/urandom");

    struct bStream *stream = bsopen((bNread)fread, urand);
    check(stream != NULL, "Failed to open /Users/baz/VSProjects/zedc/c-skeleton/urandom");
    ...
    for(i = 0; i < num_keys; i ++) {
        ...
        check(rc >= 0, "Failed to read from /Users/baz/VSProjects/zedc/c-skeleton/urandom.");
...
char *test_distribution(){
    ...
    fill_distribution(stats[ALGO_DJB], keys, Hahsmap_djb_hash);
